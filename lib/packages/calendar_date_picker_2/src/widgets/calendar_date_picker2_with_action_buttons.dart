  ...
  void didUpdateWidget(covariant CalendarDatePicker2WithActionButtons oldWidget) {
    ...
    if (isValueSame) {
      for (var i = 0; i < oldWidget.initialValue.length; i++) {
        var isSame = (oldWidget.initialValue[i] == null && widget.initialValue[i] == null) ||
            DateUtils.isSameDay(oldWidget.initialValue[i], widget.initialValue[i]);
        if (!isSame) {
          isValueSame = false;
          break;
        }
  ...
  Widget _buildCancelButton(ColorScheme colorScheme) {
    return InkWell(
      ...
      onTap: () => setState(() {
        ...
        if ((widget.config.openedFromDialog ?? false) && (widget.config.shouldCloseDialogAfterCancelTapped ?? false)) {
